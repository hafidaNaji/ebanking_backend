import { Injectable } from '@angular/core';
import {HttpClient} from '@angular/common/http';
import {Observable} from 'rxjs';
import {Customer} from '../model/customer.model';
import {environment} from '../../environments/environment';


@Injectable({
  providedIn: 'root'
})
export class CustomerService {
   //backendHost:string="http://localhost:8085"
    constructor(private http:HttpClient) { }

  public getCustomers():Observable<Array<Customer>>{
   //return this.http.get<Array<Customer>>("http://localhost:8085/customers")
   // return this.http.get<Array<Customer>>(this.backendHost+"/customers")
    return this.http.get<Array<Customer>>(environment.backendHost+"/customers")
  }

  public searchCustomers(keyword :string):Observable<Array<Customer>>{
   //return this.http.get<Array<Customer>>("http://localhost:8085/customers/search?keyword="+keyword)
    //return this.http.get<Array<Customer>>(this.backendHost+"/customers/search?keyword="+keyword)
    return this.http.get<Array<Customer>>(environment.backendHost+"/customers/search?keyword="+keyword)
  }

  public saveCustomer(customer:Customer):Observable<Customer>{
    //return this.http.post<Customer>("http://localhost:8085/customers")
    //return this.http.post<Customer>(this.backendHost+"/customers")
    return this.http.post<Customer>(environment.backendHost+"/customers",customer);
  }

  public deleteCustomer(id: number){
    //return this.http.delete("http://localhost:8085/customers,customer")
    //return this.http.delete(this.backendHost+"/customers,customer")
    return this.http.delete(environment.backendHost+"/customers/"+id);
  }


}
